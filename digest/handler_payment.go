...
func (hd *Handlers) handlePaymentDesignInGroup(contract string) ([]byte, error) {
	...
	de, st, err := pmdigest.PaymentDesign(hd.database, contract)
	if err != nil {
		return nil, err
	}
...
func (hd *Handlers) buildPaymentDesign(contract string, de types.Design, st base.State) (cdigest.Hal, error) {
	h, err := hd.combineURL(pmdigest.HandlerPathPaymentDesign, "contract", contract)
	if err != nil {
		return nil, err
	}
...
func (hd *Handlers) handlePaymentAccountInfoInGroup(contract, account string) ([]byte, error) {
	var accountInfoValue *pmdigest.AccountInfoValue

	accountInfoValue, err := pmdigest.AccountInfo(hd.database, contract, account)
	if err != nil {
		return nil, err
	}
...
func (hd *Handlers) buildAccountInfoValue(contract string, it pmdigest.AccountInfoValue) (cdigest.Hal, error) {
	h, err := hd.combineURL(
		pmdigest.HandlerPathPaymentAccountInfo,
